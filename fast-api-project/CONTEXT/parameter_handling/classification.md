---
description: How analyze_param() decides if a parameter is path, query, body, header, cookie, or a dependency — the classification rules
---

# Parameter Classification

`analyze_param()` in `dependencies/utils.py` classifies each endpoint parameter:

## Decision order

1. **Annotated with Depends()** → sub-dependency (not a param, recurse into it)
2. **Annotated with Body/Form/File** → body parameter
3. **Annotated with Header** → header parameter (underscores converted to dashes)
...
This is the non-obvious part. A parameter with no annotation marker gets classified
by its type:
...
`analyze_param()` unpacks `Annotated` and finds the `FieldInfo` subclass in metadata.
Multiple `FieldInfo` in one `Annotated` is an error.
