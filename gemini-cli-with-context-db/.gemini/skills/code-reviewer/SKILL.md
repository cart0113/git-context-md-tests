...
1.  **Checkout**: Use the GitHub CLI to checkout the PR.
    ...
    ```
2.  **Preflight**: Execute the project's standard verification suite to catch
    automated failures early.
    ```bash
    npm run preflight
    ```
...
2.  **Preflight (Optional)**: If the changes are substantial, ask the user if
    they want to run `npm run preflight` before reviewing.
...
- After the review, ask the user if they want to switch back to the default
